...
<div>
    ...
    <form action="{{ route('new_role') }}" method="get" class="inline-block">
        @csrf
    ...
    <div class="mt-6">
        <fieldset class="border border-solid border-black p-3">
            ...
            <form action="{{ route('roles') }}" method="GET">
                ...
                <input class="m-1" type="text" name="name" placeholder="{{ __('Name') }}">
    ...
    <div class="mt-5">
        <x-roles-table table="roles" :items="$roles" :queryString="$query_string"/>
    </div>
